...
class TextSearch:
    ...
    def asr_retrieval_helper(self, asr_input, k, storage, index, semantic, keyword):
        ...
        if storage == "cloud": 
            infos_query = list(map(self.json_path_cloud.get, list(asr_idx_image)))
        else: 
            infos_query = list(map(self.json_path.get, list(asr_idx_image)))
        image_paths = [(info['image_path']) for info in infos_query]
        return asr_scores, infos_query, image_paths

    def ocr_search(self, text, top_k, storage):
        scores, sort_index = self.ocr_retrieval(text, top_k)
        if storage == "cloud": 
            infos_query = list(map(self.json_path_cloud.get, list(sort_index)))
        else: 
            infos_query = list(map(self.json_path.get, list(sort_index)))
        image_paths = [(info['image_path']) for info in infos_query]
...
if __name__ == "__main__":
    ...
    _, _, image_paths = search.asr_retrieval_helper("lễ hội", 5, "local", None, True, True)
